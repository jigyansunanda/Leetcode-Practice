def solve(index, hold, transaction, prices, dp):
    if transaction >= 2:
        return 0
    if index >= len(prices):
        return 0
    ...
    if hold == 1:
        keepHolding = solve(index + 1, hold, transaction, prices, dp)
        sell = solve(index + 1, 0, transaction + 1, prices, dp) + prices[index]
        dp[index][hold][transaction] = max(keepHolding, sell)
    else:
        buy = solve(index + 1, 1, transaction, prices, dp) - prices[index]
        dontBuy = solve(index + 1, 0, transaction, prices, dp)
